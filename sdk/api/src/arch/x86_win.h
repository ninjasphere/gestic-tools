 ...
 *
 ******************************************************************************/
#ifndef GESTIC_ARCH_WINDOWS_H
#define GESTIC_ARCH_WINDOWS_H
...
#include <windows.h>
#ifndef GESTIC_SLEEP
#   define GESTIC_SLEEP(MS) Sleep(MS)
#endif

#if defined(GESTIC_SYNC_INTERRUPT)
#   error "Interrupt-based message handling synchronization not supported on Windows."
#elif defined(GESTIC_SYNC_THREADING)
#   if !defined(GESTIC_SYNC_INIT) || !defined(GESTIC_SYNC_LOCK) || !defined(GESTIC_SYNC_UNLOCK) || !defined(GESTIC_SYNC_RELEASE)
#       define GESTIC_SYNC_INIT(SYNC) (*(HANDLE*)(SYNC) = CreateMutex(NULL, FALSE, NULL))
#       define GESTIC_SYNC_LOCK(SYNC) WaitForSingleObject((HANDLE)SYNC, INFINITE)
#       define GESTIC_SYNC_UNLOCK(SYNC) ReleaseMutex((HANDLE)SYNC)
#       define GESTIC_SYNC_RELEASE(SYNC) CloseHandle((HANDLE)SYNC)
#   endif
#endif
